...
class SystemFuncsModel:
    ...
    def copia_para(self,item,root_path,second_path):
    ...
    def copia_para_binary(self,item,root_path,second_path):
        lista=listdir(second_path)
    ...
    def verify_dependecies(self,path):
    ...
    def organize_pasta(self):
        organize_desktop()
        organize_downloads()
        organize_documents()

    def get_takList(self):
    ...
    def busca(self,alvo,raiz,lista=[]):
        current_path=raiz
    ...
    def search_local(self,alvo,auto_correct=True):
        if auto_correct:
            aux=alvo[::-1]
            aux=aux.replace(' .','.')
            aux=aux[:aux.find(' ')]
