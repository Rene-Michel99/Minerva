...
from Modules.Decorators import Manage_erros
...
class Core:
    def __init__(self):
        ...
        self.occupied = []
        self.dic_funcs = {
            '<time>':self.time,'<date>':self.date,'<get_errors>':self.status,'<math>':self.math,'<s_yt>':self.open_youtube,
            '<open>':self.open_app,'<news>':self.get_news,'<s_zoom>':self.search_zoom,'<g_tasks>':self.g_tasks,
            '<screen_cpt>':self.screen_cpt,'<next_music>':self.next_music,'<battery>':self.get_battery,
            '<lembrete>':self.create_reminder,'<get_ocupied>':self.get_ocupied,'<get_performance>':self.get_performance,
            '<weather>':self.get_weather,'<s_wiki>':self.search_wiki,'<s_local>':self.search_local
            }
    ...
    def time(self,*kwargs):
        ...
        st = Aux_functions.get_the_status(time[0:2])
    ...
    def date(self,*kwargs):
        string = Aux_functions.translate(datetime.datetime.now().strftime("%A:%d:%b"))
        return string
    ...
    def math(self,*kwargs):
        res = kwargs[0]
        resp = 'O resultado é '+str(Aux_functions.solve_math(res))
        return resp
    ...
    def search_zoom(self,*kwargs):
        ...
        res = Aux_functions.three_more_cheap(res)
        print(res)
        return res
    ...
    @Manage_erros.get_error
    def scan_img(self,*kwargs):
        self.engine_vstt.speak('Qual o nome da imagem que eu devo escaneiar?')
        x = self.engine_vst.take_command()
        x = x+'.png'
        self.engine_vstt.speak('Aguarde um momento')
        path=Sys_funcs.search_local(x,False)
        x=0
        if type(path)==str:
            self.engine_vstt.speak(path)
            return 0
        if type(path)!=str and len(path)>1:
            self.engine_vstt.speak('Há mais de uma imagem com esse nome, diga qual você escolhe')
            for i in range(len(path)):
                print(i,'. ',path[i])
            x=int(self.engine_vstt.take_command)
        frase=Image_processing.scan_image(path[x])
        self.engine_vstt.speak(frase)
    ...
    def create_reminder(self,*kwargs):
        ...
        data = Aux_functions.transform_in_date(data)
